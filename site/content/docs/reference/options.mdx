...
`fuse mcp serve` takes no command options. `fuse mcp install` accepts `--client`, `--scope`, `--command`, `--no-rules`, and `--with-hooks`. `fuse mcp doctor` accepts `--client`, `--scope`, `--with-hooks`, and `--json`. See [Commands](/docs/reference/commands) for behavior.
