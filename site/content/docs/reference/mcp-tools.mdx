...
| `building_syntax` | A syntax job is inventorying or extracting source. The caller can wait, inspect status, or explicitly cancel the shared job. |
| `upgrade_pending` | A readable syntax or partial index exists while an explicit semantic job adds compiler facts. |
| `ready` | The index is current for reads at its loaded tier. |
| `index_busy` | An external writer or SQLite lock blocked the read. Daemon-owned index requests join the shared job instead of returning this state. |
