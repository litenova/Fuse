...
public sealed class FileCollector : IFileCollector
{
    ...
    public List<FileProcessingInfo> CollectFiles(FuseOptions options, ResolvedConfiguration config)
    {
        ...
        return _fileSystem.EnumerateFiles(options.SourceDirectory, "*.*", searchOption)
            ...
            // Filter 7: Skip empty (zero-byte) files if requested
            .Where(f => !options.ExcludeEmptyFiles || f.Info.Length > 0)

            // Filter 8: Skip auto-generated files if requested
            .Where(f => !options.ExcludeAutoGenerated || !IsAutoGenerated(f.Path))

            // Filter 9: Apply file name exclusions (exact match)
            .Where(f => !IsExcludedByFileName(f.Info.Name, options.ExcludeFiles))

            // Filter 10: Apply file pattern exclusions (file name and relative path)
            .Where(f => !IsExcludedByPattern(f.Info.Name, f.RelativePath, config.ExcludePatterns))
    ...
    /// <summary>
    ///     Determines if a file name exactly matches any of the specified exclusion names.
    /// </summary>
    /// <param name="fileName">The file name to check.</param>
    /// <param name="excludeFiles">The collection of file names to exclude, or <c>null</c>.</param>
    /// <returns><c>true</c> if the file name is in the exclusion list; otherwise, <c>false</c>.</returns>
    private static bool IsExcludedByFileName(string fileName, string[]? excludeFiles)
    {
        if (excludeFiles == null || excludeFiles.Length == 0) return false;
        return excludeFiles.Contains(fileName, StringComparer.OrdinalIgnoreCase);
    }

    /// <summary>
    ///     Determines if a file matches any of the exclusion patterns by name or relative path.
    /// </summary>
    /// <param name="fileName">The file name to check.</param>
    /// <param name="relativePath">The relative path to check (supports path-based glob patterns).</param>
    ...
    private static bool IsExcludedByPattern(string fileName, string relativePath, IReadOnlyCollection<string> excludePatterns)
    {
        if (excludePatterns.Count == 0) return false;

        var normalizedRelPath = relativePath.Replace(Path.DirectorySeparatorChar, '/');

        foreach (var pattern in excludePatterns)
        {
            var glob = Glob.Parse(pattern);
            if (glob.IsMatch(fileName) || glob.IsMatch(normalizedRelPath))
                return true;
        }
    ...
    /// <summary>
    ///     Determines if a file contains an auto-generated code marker in its first few lines.
    /// </summary>
    /// <param name="filePath">The full path to the file.</param>
    /// <returns><c>true</c> if the file appears to be auto-generated; otherwise, <c>false</c>.</returns>
